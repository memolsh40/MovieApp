...
export const MovieList = () => {
  ...
  useEffect(() => {
    ...
    if (genre) {
      if (genre === 'ایرانی') {
        url = `http://localhost:8080/api/movies/iranianM?genre=${genre}&pageNumber=${currentPage}&pageSize=12`;
        setTitle(`فیلم های ${genre}`);
      } else {
        url = `http://localhost:8080/api/media/genre/${genre}?pageNumber=${currentPage}&pageSize=12`;
        setTitle(`فیلم های ${genre}`);
      }
    } else {
      url = `http://localhost:8080/api/movies/getAll?pageNumber=${currentPage}&pageSize=12`;
      setTitle('فیلم ها');
    }
